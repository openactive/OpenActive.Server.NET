...
namespace BookingSystem
{
    public class AcmeOrdersFeedRpdeGenerator : OrdersRPDEFeedModifiedTimestampAndID
    {
        ...
        protected async override Task<List<RpdeItem>> GetRPDEItems(OrderType feedType, string clientId, long? afterTimestamp, string afterId)
        {
            if (!(feedType == OrderType.Order || feedType == OrderType.OrderProposal)) throw new ArgumentOutOfRangeException(nameof(feedType));
            using (var db = FakeBookingSystem.Database.Mem.Database.Open())
            {
                // Note two queries are only required when implementing the Approval flow, otherwise only an Orders feed is required
                var q = (feedType == OrderType.Order ?
                    db.From<OrderTable>()
